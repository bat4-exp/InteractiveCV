...
function getChatInstructions(): string {
  try {
    const filePath = join(process.cwd(), 'content', 'chat-instructions.md');
...
function getResumeContext(): string {
  try {
    const filePath = join(process.cwd(), 'content', 'resume-context.md');
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // CORS: allow preflight (OPTIONS) so cross-origin POST works
